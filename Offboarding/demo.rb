# demo.rb - generate mail send in JSON,cURL, or make HTTP req
require 'httparty'
require 'json'
require "neatjson"
...
puts subject
puts "Enter text as body content or a Template ID: " 
content = gets.chomp
if (/[d]\-\S*/ =~ content)
    content = "\"template_id\": \"#{content}\""
else
content = "\"content\": [{\"type\": \"text/html\", \"value\": \"<p>#{content.gsub(/\"/, "\\\"")} </p> \"} ]"
end
...
if (send_at == "now")
  #Build Payload
  payload = "{\"personalizations\": [#{to_p}], \"from\": {\"email\": \"#{from}\"}, \"subject\": \"#{subject}\", #{content} , \"categories\": [\"new_sign_up\"], \"custom_args\": {\"user\":\"subuser_username\", \"dispatch_id\":\"#{rand(1000..9999)}\"}}"
  #puts "Here is you v3 payload (already copied to your clipboard): "
  # print payload an load to clipboard
else
  #Build Payload
  payload = "{\"personalizations\": [#{to_p}], \"from\": {\"email\": \"#{from}\"}, \"subject\": \"#{subject}\", #{content}, \"categories\": [\"new_sign_up\"], \"custom_args\": {\"user\":\"subuser_username\", \"dispatch_id\":\"#{rand(1000..9999)}\"}, \"send_at\": \"#{send_at}\" }"
  #puts "Here is you v3 payload (already copied to your clipboard): "
  # print payload an load to clipboard
end
...
if (type == "curl")
  ...
  if getsub.code == 200
    subuser_username =  getsub["username"]
    payload = payload.gsub!(/subuser_username/ , subuser_username) 
  end
...
if (type == "json")
  ...
  #ap payload
  if(copy =="y")
    pbcopy(JSON.pretty_generate(payload))
  end
end
